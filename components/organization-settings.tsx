...
function WorkspacePage({ section, onNavigate }: { section: string; onNavigate: (value: string) => void }) {
  const copy: Record<string, { eyebrow: string; title: string; description: string; items: string[] }> = {
    Organisations: { eyebrow: 'Workspace', title: 'Organisation settings', description: 'Manage your organisation profile, access policies, domains, and security controls.', items: ['Acme Inc. · Pro plan', 'Organisation profile and metadata', 'Access control and permission policies', 'Domains and discoverability'] },
    Teams: { eyebrow: 'Workspace', title: 'Teams', description: 'Create teams, assign members, and manage team-level access to delivery resources.', items: ['Web · 8 members · 12 repositories', 'Platform · 5 members · 6 repositories', 'Infrastructure · 4 members · 9 repositories', 'Create a team or review team permissions'] },
    Members: { eyebrow: 'Workspace', title: 'Organisation members', description: 'Review members, roles, permissions, and recent activity across the organisation.', items: ['Olivia Rhye · Owner', 'Phoenix Baker · Admin', 'Lana Steiner · Member', 'Demi Wilkinson · Member'] },
    Invitations: { eyebrow: 'Workspace', title: 'Invitations', description: 'Track pending invitations and manage access requests sent to your organisation and teams.', items: ['maya@northstar.dev · Web · Admin · Pending', 'sam@northstar.dev · Platform · Member · Pending', '2 invitations expire within 5 days', 'Invite a member with custom permissions'] },
    Settings: { eyebrow: 'Workspace', title: 'Workspace settings', description: 'Configure notifications, audit logs, API access, and organisation-wide defaults.', items: ['Notifications and delivery alerts', 'Audit log retention · 90 days', 'API keys and connected integrations', 'Danger zone and organisation deletion'] },
  }
  const data = copy[section] ?? copy.Organisations
  return <div className="flex flex-col gap-6"><Header eyebrow={data.eyebrow} title={data.title} description={data.description} action={<Button onClick={() => toast.success(`${section} action opened`)}><Users data-icon="inline-start" />{section === 'Invitations' ? 'Invite member' : section === 'Teams' ? 'Create team' : 'Manage access'}</Button>} /><div className="grid gap-4 sm:grid-cols-2"><Card><CardHeader><CardTitle>Quick access</CardTitle><CardDescription>Common actions for this workspace area.</CardDescription></CardHeader><CardContent className="flex flex-col gap-2">{data.items.slice(0, 2).map((item) => <Button key={item} variant="outline" className="justify-start" onClick={() => toast.success('Action ready')}>{item}</Button>)}</CardContent></Card><Card><CardHeader><CardTitle>Recent activity</CardTitle><CardDescription>Latest changes in {section.toLowerCase()}.</CardDescription></CardHeader><CardContent className="flex flex-col gap-3">{data.items.slice(2).map((item) => <div key={item} className="rounded-lg border bg-muted/30 p-3 text-sm">{item}</div>)}</CardContent></Card></div><Card><CardHeader><CardTitle>{section} overview</CardTitle><CardDescription>Use the actions above to manage access and configuration.</CardDescription></CardHeader><CardContent className="grid gap-3 md:grid-cols-3">{['Access', 'Activity', 'Policies'].map((label) => <button type="button" key={label} onClick={() => toast.success(`${label} panel opened`)} className="rounded-xl border p-4 text-left transition-colors hover:bg-muted"><p className="font-medium">{label}</p><p className="mt-1 text-sm text-muted-foreground">Review {label.toLowerCase()} for this workspace.</p></button>)}</CardContent></Card></div>
}
...
export default function OrganizationSettings() {
  ...
  const main = active === 'Overview' ? <Overview onNavigate={navigate} /> : active === 'Builds' ? <Builds onNavigate={navigate} /> : active === 'Build detail' ? <BuildDetail onNavigate={navigate} /> : active === 'Billing' ? <BillingReport /> : <WorkspacePage section={active} onNavigate={navigate} />
