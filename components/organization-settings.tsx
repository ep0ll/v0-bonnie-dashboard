...
function Sidebar({ active, onNavigate }: { active: string; onNavigate: (value: string) => void }) {
  ...
  return <><aside className="hidden h-screen w-64 shrink-0 lg:block">{content}</aside><Sheet><SheetTrigger asChild><Button variant="outline" size="icon" className="fixed left-4 top-4 z-20 lg:hidden" aria-label="Open navigation"><Menu /></Button></SheetTrigger><SheetContent side="left" className="w-72 p-0"><SheetTitle className="sr-only">Navigation</SheetTitle>{content}</SheetContent></Sheet></>
}
